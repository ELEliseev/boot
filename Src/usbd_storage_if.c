...
USBD_StorageTypeDef USBD_Storage_Interface_fops_FS = { STORAGE_Init_FS,
		STORAGE_GetCapacity_FS, STORAGE_IsReady_FS, STORAGE_IsWriteProtected_FS,
		STORAGE_Read_FS, STORAGE_Write_FS, STORAGE_GetMaxLun_FS,
...
int8_t STORAGE_Init_FS(uint8_t lun) {
	/* USER CODE BEGIN 2 */
	//flashErase(FLASH_DISK_START_ADDRESS);
	//flashErase(FLASH_DISK_START_ADDRESS+1024);
	return (USBD_OK);
	/* USER CODE END 2 */
}
...
//		case 0:
//			HAL_FLASH_Unlock();
//			for(uint32_t k = 0; k < blk_len; k++)
//			{
//				for(uint32_t i = 0; i < STORAGE_BLK_SIZ; i++)
//				{
//					buf[i] = ((volatile uint8_t*)(FLASH_DISK_START_ADDRESS + (blk_addr*STORAGE_BLK_SIZ)))[i];
//				}
//			}
//			break;
//		case 1:
